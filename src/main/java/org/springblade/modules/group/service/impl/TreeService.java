...
public class TreeService {
    ...
    public void transformDeptTree(List<GroupDto> gruopLevelList, String level, Multimap<String, GroupDto> levelGroupMap) {
        for (int i = 0; i < gruopLevelList.size(); i++) {
			...
			companyList= companyMapper.getListTbCompany(groupLevelDto.getId());
			if(CollectionUtils.isNotEmpty(companyList)){

				for(int j=0;j<companyList.size();j++){
					Company company =companyList.get(j);
					GroupDto tbGroup=new GroupDto();
					tbGroup.setId(company.getId());
					tbGroup.setGname(company.getCname());
					tbGroup.setIsgroup("1");
					tbGroup.setAbout(company.getCcontent());
					tempGroupList.add(tbGroup);
				}
				groupLevelDto.setGroupList(tempGroupList);
			}
    ...
    //子分组排序
    public Comparator<GroupDto> groupSeqComparator = new Comparator<GroupDto>() {
        public int compare(GroupDto o1, GroupDto o2) {
            return o1.getSeq() - o2.getSeq();
        }
    };
